...
describe('AFiBase', () => {
    ...
    it("withdraw check", async () => {
        ...
        let  dolomiteBalance = await aFiStorageInstance.balanceDolomite(aTokenConInstance.address, 0);
        console.log("before Advance dolomiteBalance ", dolomiteBalance);
     
        // Advance time by 2 weeks (14 days)
        await network.provider.send("evm_increaseTime", [1 * 24 * 60 * 60]); // 14 days in seconds
        await network.provider.send("evm_mine"); // Mine a new block with the new timestamp
        
        dolomiteBalance = await aFiStorageInstance.balanceDolomite(aTokenConInstance.address, 0);
        console.log("after Advance dolomiteBalance", dolomiteBalance);
        ...
        dolomiteBalance = await aFiStorageInstance.balanceDolomite(aTokenConInstance.address, 0);
        console.log("after withdraw after dolomiteBalance", dolomiteBalance);
