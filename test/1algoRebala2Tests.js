...
describe('AFi Product 101', function (accounts) {
    ...
    describe('Basic checks for deposit and withdraw', () => {
        ...
        it("algo rebalance 2", async () => {
            ...
            var afterusdtBalance = await usdtConInstance.balanceOf(investor1.address)
            console.log("After Deposit user usdt balance", `${afterusdtBalance}`)

            var afterusdcBalance = await usdcConInstance.balanceOf(aTokenConInstance.address)
            console.log("After deposit user usdc balance", `${afterusdcBalance}`)

            var afibalance1 = await usdtConInstance.balanceOf(aTokenConInstance.address)
            console.log("aficontract usdt balance", `${afibalance1}`)
        ...
        it("algo rebalance and depsoits", async () => {
            snapshotId = await ethers.provider.send('evm_snapshot');
            const beforeUSDTDep = await usdtConInstance.balanceOf(investor1.address)
            console.log("before Deposit user usdt balance", `${beforeUSDTDep}`)
        ...
        it("algo rebalance and depsoits and withdraw", async () => {
            snapshotId = await ethers.provider.send('evm_snapshot');
            const beforeUSDTDep = await usdtConInstance.balanceOf(investor1.address)
            console.log("before Deposit user usdt balance", `${beforeUSDTDep}`)
            ...
            console.log("Afterbal", `${afterbal}`);
            const minimumReturnAmount1 = [0, 0, 0, 0, 0];

            const amount = minimumReturnAmount1.map(num => BigNumber.from(num));
            const returnStrings = amount.map(bn => bn.toString());
            ...
            await aTokenConInstance.connect(investor1).withdraw(
                afterbal, usdtConInstance.address, deadline, returnStrings, 3, 0
            );
        ...
        it("algo rebalance and reinitialize on the vault", async () => {
            ...
            await aFiPassiveRebalanceInstance.setStorage(aFiStorageInstance.address);
            await aFiPassiveRebalanceInstance.setOracle(aFiAFiOracleInstance.address);
            // await aFiAFiOracleInstance.increaseObservation("0x22127577D772c4098c160B49a8e5caE3012C5824", 2);
            ...
            var preswapDep = await aFiStorageInstance.getPreSwapDepositsTokens(aTokenConInstance.address, await aTokenConInstance.getcSwapCounter(), usdtConInstance.address);
            console.log("aTokenConInstance preswapdep ------------", preswapDep);

            // var balanceOFUNI = await aFiStorageInstance.balance("0xFa7F8980b0f1E64A2062791cc3b0871572f1F7f0", aTokenConInstance.address);
            // console.log("balanceOFUNI ------------", balanceOFUNI);

            // var balanceOFUNI = await aFiStorageInstance.balance("0xf97f4df75117a78c1A5a0DBb814Af92458539FB4", aTokenConInstance.address);
            // console.log("balanceOFLINK ------------", balanceOFUNI);
            const reinitData = {
                uTokens: [
                    "0xFa7F8980b0f1E64A2062791cc3b0871572f1F7f0",  // UNI
                    "0xf97f4df75117a78c1A5a0DBb814Af92458539FB4", // LINK
                ],
                oracles: oraclePayload,  // your oracle addresses array
                defaultProp: [5000000, 5000000],
                currentProp: [5000000, 5000000]
            };
            ...
            preswapDep = await aFiStorageInstance.getPreSwapDepositsTokens(aTokenConInstance.address, await aTokenConInstance.getcSwapCounter(), usdtConInstance.address);
            console.log("aTokenConInstance preswapdep ------------", preswapDep);

            // balanceOFUNI = await aFiStorageInstance.balance("0xFa7F8980b0f1E64A2062791cc3b0871572f1F7f0", aTokenConInstance.address);
            // console.log("balanceOFUNI ------------", balanceOFUNI);

            // balanceOFUNI = await aFiStorageInstance.balance("0xf97f4df75117a78c1A5a0DBb814Af92458539FB4", aTokenConInstance.address);
            // console.log("balanceOFLINK ------------", balanceOFUNI);
        ...
        it("algo rebalance and reinitialize on the vault when fees is not zero", async () => {
            snapshotId = await ethers.provider.send('evm_snapshot');
            const beforeUSDTDep = await usdtConInstance.balanceOf(investor1.address)
            console.log("before Deposit user usdt balance", `${beforeUSDTDep}`)
